...
| Component | Notes |
|---|---|
| **Raspberry Pi 5** | 64-bit, running Ubuntu 24.04 LTS (Noble Numbat). Verify with `uname -m` — must report `aarch64`. |
...
| **ReSpeaker Mic Array v2.0** | 4 mics + AEC reference (6 channels total). For the multimodal sensing layer. |
...
System packages the above depend on:
...
## 7. Building the `desktoprobo` Workspace
...
## 9. Motor Configuration Reference
